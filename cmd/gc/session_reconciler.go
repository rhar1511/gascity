	...
	for _, target := range wakeTargets {
		...
		if shouldWake && target.alive {
			...
			if beginIdleRespawnDrainIfIdle(target.info, eval, dt, sp, clk) {
				...
				fmt.Fprintf(stdout, "Draining session '%s': %s\n", target.info.SessionName, idleRespawnDrainReason) //nolint:errcheck
				if trace != nil {
					trace.RecordDecision(TraceSiteReconcilerDrainDecision, TraceReasonCode(idleRespawnDrainReason), TraceOutcomeDrain, target.tp.TemplateName, target.info.SessionName, traceRecordPayload{})
				}
...
func beginIdleRespawnDrainIfIdle(info sessionpkg.Info, eval wakeEvaluation, dt *drainTracker, sp runtime.Provider, clk clock.Clock) bool {
	if strings.TrimSpace(info.ID) == "" || !idleAssignedWorkOnly(eval) {
		return false
	}
	...
	// shouldBeginIdleDrainInfo short-circuits true for them without a probe).
	// Both keep their existing "stay awake with assigned work" behavior.
	if isNamedSessionInfo(info) || eval.Policy.Class == config.SessionSleepNonInteractive {
		return false
	}
	ok, err := shouldBeginIdleDrainInfo(info, eval, dt, sp)
	if err != nil || !ok {
		return false
	}
