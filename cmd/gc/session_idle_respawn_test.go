...
func TestSelectIdleProbeTargets_IncludesAssignedWorkOnly(t *testing.T) {
	...
	target := wakeTarget{
		info:  sessionpkg.Info{ID: "s1", SessionName: "run-operator-1", SessionNameMetadata: "run-operator-1"},
		alive: true,
	}
	wakeEvals := map[string]wakeEvaluation{
		"s1": {Reason: "assigned-work", Reasons: []WakeReason{WakeWork}, Policy: policy},
	}
	infoByID := map[string]sessionpkg.Info{"s1": target.info}
	dt := newDrainTracker()
...
func TestSelectIdleProbeTargets_SkipsOtherWakeReasons(t *testing.T) {
	...
	target := wakeTarget{
		info:  sessionpkg.Info{ID: "s1", SessionName: "worker-1", SessionNameMetadata: "worker-1"},
		alive: true,
	}
...
func TestBeginIdleRespawnDrainIfIdle(t *testing.T) {
	...
	session := sessionpkg.Info{ID: "s1", SessionName: name, SessionNameMetadata: name, Generation: "1"}
...
// Non-interactive sessions short-circuit shouldBeginIdleDrainInfo to true
// without a probe; they must be excluded from idle-respawn so an assigned
// non-interactive (e.g. named) session is not wrongly drained.
func TestBeginIdleRespawnDrainIfIdle_SkipsNonInteractive(t *testing.T) {
	...
	session := sessionpkg.Info{ID: "ni", SessionName: name, SessionNameMetadata: name, Generation: "1"}
	eval := wakeEvaluation{
